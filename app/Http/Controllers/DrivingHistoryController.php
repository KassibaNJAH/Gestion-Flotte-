...
class DrivingHistoryController extends Controller
{
    ...
    {
        try {
            $histories = DrivingHistory::with(['driver', 'vehicule'])->latest()->get();
    ...
    public function show(DrivingHistory $history)
    {
        return response()->json([
            'success' => true,
            'data' => $history->load(['driver', 'vehicule'])
        ]);
    }
    ...
    public function update(Request $request, DrivingHistory $history)
{
    try {
        ...
        if (strlen($validated['type']) > 100) {
            return response()->json([
                'success' => false,
                'message' => 'Le type ne doit pas dépasser 100 caractères'
            ], 422);
        }
        ...
        if (str_contains($e->getMessage(), 'Data truncated for column')) {
            $message = 'La valeur fournie est trop longue pour un des champs';
        }
