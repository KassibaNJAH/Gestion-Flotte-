    ...
    public function index()
    {
        try {
            return response()->json([
                'success' => true,
                'data' => Driver::with('vehicules')->get()
            ]);
        } catch (\Exception $e) {
    ...
    public function store(Request $request)
{
    ...
    public function assignVehicle(Request $request, Driver $driver)
    {
        try {
            ...
            $driver->vehicules()->syncWithoutDetaching([
                $validated['vehicule_id'] => ['assigned_at' => now()]
            ]);
            ...
            Log::error('DriverController@assignVehicle - ' . $e->getMessage());
            return response()->json([
                'success' => false,
                'message' => 'Erreur lors de l\'attribution du véhicule'
            ], 500);
        }
    }
