...
class AssignementController extends Controller
{
    ...
    public function index()
    {
        return response()->json(
            Assignment::with(['vehicule', 'driver'])->get()
        );
    }
   ...
   public function store(Request $request)
{
    ...
    if (!$vehicule->isAvailableBetween($validated['start_date'], $validated['end_date'])) {
    ...
    return response()->json($assignment->load(['vehicule', 'driver']), 201);
}
    ...
    {
        $assignment = Assignment::with(['vehicule', 'driver'])->find($id);
    ...
    public function update(Request $request, string $id)
{
    ...
    if (isset($validated['vehicule_id'])) {
        ...
        if (!$vehicule->isAvailableBetween(
    ...
    return response()->json($assignment->load(['vehicule', 'driver']));
}
